                            ...
                            std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> &homography_builder)
{
    ...
    std::vector<cv::Mat> Hs(n_images);
    {
        ...
        std::vector<int> stack(n_images);
        for (int i = 0; i < n_images; ++i) stack[i] = i;

        std::vector<bool> found(n_images, false);
        while (!stack.empty()) {
            int cur = stack.back();
            ...
            if (curParent == -1) {
                Hs[cur] = cv::Mat::eye(3, 3, CV_64F);
                found[cur] = true;
                stack.pop_back();
            } else if (found[curParent]) {
                Hs[cur] = Hs[curParent] * homography_builder(imgs[cur], imgs[curParent]);
                found[cur] = true;
                stack.pop_back();
            } else {
