                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (const auto &m : matches) {
        if (m.size() <= 1 || m[0].distance / m[1].distance < 0.6) {
...
//    метч остается, если левое и правое множества первых total_neighbors соседей в радиусах поиска(radius2_query, radius2_train) имеют как минимум consistent_matches общих элементов
    for (int i = 0; i < n_matches; ++i) {
        std::set<int> indices_query_neighbours, indices_train_neighbours;
        for (int j = 0; j < total_neighbours; ++j) {
            if (distances2_query.at<float>(i, j) <= radius2_query) {
                indices_query_neighbours.insert(indices_query.at<int>(i, j));
            }
            if (distances2_train.at<float>(i, j) <= radius2_train) {
                indices_train_neighbours.insert(indices_train.at<int>(i, j));
            }
        }

        std::vector<int> intersections;
        std::set_intersection(indices_query_neighbours.begin(), indices_query_neighbours.end(),
                              indices_train_neighbours.begin(), indices_train_neighbours.end(),
                              std::back_inserter(intersections));

        if (intersections.size() >= consistent_matches) {
